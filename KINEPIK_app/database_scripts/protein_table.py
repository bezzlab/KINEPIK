...
def collectProteins():
    '''The function collects all the protein infos from other tables and theb returns them as a list to the user'''
    # selecting all the proteins from the tables that have proteins
    inter_s = session.query(Interaction.source).distinct().all()
    inter_t = session.query(Interaction.target).distinct().all()
    effect_s = session.query(Effect.source).distinct().all()
    effect_t = session.query(Effect.target).distinct().all()
    pert_inter = session.query(Perturbation_interaction.target).distinct().all()
    phos = session.query(Phosphosite.uniprot_id).distinct().all()
    s_loc = session.query(Subcell_location.protein).distinct().all()
    t_loc = session.query(Tissue_location.protein).distinct().all()
    
    # combining all of them into one list to be handled further
    all_proteins = inter_s + inter_t + effect_s + effect_t + pert_inter + phos + s_loc + t_loc
